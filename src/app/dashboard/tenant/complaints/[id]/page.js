...
export default function TenantComplaintDetailPage({ params }) {
  const { id } = use(params);
  const { user } = useAuth();
  ...
  const fetchData = () => {
    fetch(`/api/complaints/${id}`)
      ...
      .then((data) => {
        ...
        if (data.complaint?.satisfaction_rating) {
          setRating(data.complaint.satisfaction_rating);
  ...
  const submitRating = async (value) => {
    ...
    try {
      await fetch(`/api/complaints/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ satisfactionRating: value }),
      });
      setRatingSubmitted(true);
    } catch {
      // ignore
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-border bg-surface-white p-3.5 shadow-soft">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted">Raised</p>
          <p className="mt-1 text-sm font-medium text-heading">{timeAgo(complaint.created_at)}</p>
          <p className="text-[10px] text-muted">{new Date(complaint.created_at).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</p>
        </div>
      </div>

      {/* Complaint details */}
      <div className="mb-6 rounded-2xl border border-border bg-surface-white p-6 shadow-soft">
        <h2 className="mb-3 text-sm font-semibold text-heading uppercase tracking-wider">Description</h2>
        ...
        {complaint.resolved_at && (
          <div className="mt-4 flex items-center gap-2 text-xs text-green-600">
            ...
            Resolved on {new Date(complaint.resolved_at).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}
      ...
      {/* Satisfaction rating & Reopen */}
      {(complaint.status === "resolved" || complaint.status === "closed") && (
        <div className="mb-6 rounded-2xl border border-border bg-surface-white p-6 shadow-soft">
          <div className="flex items-start justify-between">
            <div>
              <p className="mb-3 text-sm font-semibold text-heading">How was the resolution?</p>
              <div className="flex gap-1.5">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    className={`text-3xl transition-all ${star <= rating ? "text-yellow-400 scale-110" : "text-gray-300"} ${!ratingSubmitted && "hover:text-yellow-400 hover:scale-125 cursor-pointer"}`}
                  >
                    ★
                  </button>
                ))}
            ...
            <button
              onClick={async () => {
                if (!confirm("Reopen this complaint? This will reset the resolution.")) return;
                try {
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-orange-300 bg-orange-50 px-4 py-2 text-sm font-medium text-orange-700 transition-all hover:bg-orange-100"
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182M2.985 19.644l3.181-3.182" />
              </svg>
      ...
      <div className="rounded-2xl border border-border bg-surface-white p-6 shadow-soft">
        ...
        {comments.length === 0 ? (
          ...
          <div className="mb-4 space-y-3">
            {comments.map((c) => (
              <div key={c.id} className={`flex gap-3 ${c.user_role === "admin" ? "" : ""}`}>
                {/* Avatar */}
                ...
                <div className={`flex-1 rounded-xl px-4 py-3 ${c.user_role === "admin" ? "bg-blue-50 border border-blue-100" : c.user_name === user?.full_name ? "bg-accent-soft/20 border border-accent/10" : "bg-surface-subtle"}`}>
                  <div className="mb-1 flex items-center gap-2">
                    ...
                    {c.user_role === "owner" && (
                      <span className="rounded bg-purple-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-purple-700">Owner</span>
                    )}
        ...
        {complaint.status !== "closed" && (
          <div className="flex gap-3">
            <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${ROLE_AVATAR_COLORS.tenant}`}>
