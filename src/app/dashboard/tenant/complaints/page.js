    ...
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">Raise and track maintenance requests.</p>
        </div>
        <Link
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-border bg-surface-white p-16 text-center shadow-soft">
          ...
          <p className="text-sm font-medium text-heading">No complaints raised yet</p>
          <p className="mt-1 text-xs text-muted">All clear! Raise a complaint if something needs attention.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {complaints.map((c) => (
            <Link
              key={c.id}
              href={`/dashboard/tenant/complaints/${c.id}`}
