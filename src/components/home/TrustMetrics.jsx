...
const stats = [
  { n: '1200+', l: 'Properties Served', d: 'Across Jaipur' },
  { n: '500+', l: 'Happy Clients', d: 'Families & investors' },
  { n: '25+', l: 'Localities', d: 'In Jaipur' },
  { n: '6+', l: 'Years Experience', d: 'On-ground expertise' },
];
    ...
    <section className="relative overflow-hidden py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="mb-16 text-center">
          <p className="mb-4 text-xs font-medium tracking-elegant text-muted-on-dark">WHY CHOOSE US</p>
          <h2 className="mb-5 text-4xl font-semibold tracking-tight text-heading-on-dark md:text-5xl">
            Why Trust <span className="bg-gradient-to-r from-accent-light to-accent bg-clip-text text-transparent">DCDH Estate</span>
          </h2>
        ...
        <div className="mb-16 grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
          {stats.map((s, i) => (
            <div
              key={i}
              className="group rounded-2xl border border-border-dark bg-surface-dark-card p-8 text-center backdrop-blur-sm transition-all duration-400 hover:border-border-dark-hover hover:bg-white/8"
            >
              <div className="mb-2 text-4xl font-semibold text-heading-on-dark md:text-5xl">{s.n}</div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {points.map((p, i) => (
            <div
              key={i}
              className="group flex items-start gap-4 rounded-2xl border border-border-dark bg-surface-dark-card p-6 backdrop-blur-sm transition-all duration-400 hover:border-border-dark-hover hover:bg-white/8"
            >
              ...
              <div>
                <h4 className="mb-1 text-sm font-semibold text-heading-on-dark">{p.title}</h4>
                <p className="text-xs leading-relaxed text-body-on-dark">{p.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          <a
            ...
            className="btn-premium inline-flex items-center gap-3 rounded-full bg-surface-white px-10 py-4 text-sm font-medium text-heading transition-all duration-300 hover:bg-surface-subtle"
          >
            <svg className="h-5 w-5 text-whatsapp-dark" fill="currentColor" viewBox="0 0 24 24">
