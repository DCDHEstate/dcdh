...
export default function HowItWorks() {
  ...
  return (
    <section className="relative overflow-hidden bg-surface-white section-premium texture-grain">
      {/* Warm glow orb */}
      <div className="pointer-events-none absolute -right-64 top-1/2 h-[400px] w-[400px] rounded-full bg-accent/5 blur-[150px]" />

      <div className="relative mx-auto max-w-7xl px-4 lg:px-8">
        {/* Section Header */}
        <div className="mb-16 text-center">
          <p className="mb-4 text-xs font-medium tracking-elegant text-accent">SIMPLE & TRANSPARENT</p>
        ...
        <div className="mb-14 flex justify-center">
          <div className="inline-flex rounded-full border border-border bg-surface-white p-1 shadow-soft">
            {tabs.map(t => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                className={`rounded-full px-6 py-2.5 text-sm font-medium transition-all duration-300 ${
                  tab === t.id
                    ? 'bg-primary text-white shadow-md ring-1 ring-accent/20'
        ...
        <div className="mx-auto max-w-3xl">
          <div className="space-y-6">
            {content[tab].map((item, index) => (
              <div
                key={item.step}
                className="group flex gap-6 reveal-stagger"
                style={{ '--stagger': `${index * 150}ms` }}
              >
                {/* Step Number */}
                <div className="flex flex-col items-center">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-primary-light to-primary text-sm font-semibold text-white shadow-soft">
                    {item.step}
                  </div>
                  {item.step < 3 && (
                    <div className="mt-2 h-full w-px bg-gradient-to-b from-accent/30 via-accent/10 to-transparent" />
                  )}
                </div>
