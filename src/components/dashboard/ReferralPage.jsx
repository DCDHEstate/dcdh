...
export default function ReferralPage({ walletHref }) {
  ...
  const copyCode = () => {
    if (data?.referralCode) {
      navigator.clipboard.writeText(
        `Hey! I use DCDH Empire for zero-brokerage property search in Jaipur. Use my referral code *${data.referralCode}* when you sign up and we both earn rewards! https://dcdhempire.com`,
      );
      setCopied(true);
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Earn rewards by referring friends or property owners to DCDH Empire.
          </p>
        </div>
        <button
          onClick={() => {
      ...
      {submitMsg && (
        <div
          className={`mb-6 rounded-xl border px-4 py-3 text-sm ${submitMsg.includes("success") ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700"}`}
        >
          {submitMsg}
        </div>
      )}
        ...
        <div className="mb-6 rounded-2xl border border-accent/20 bg-gradient-to-r from-accent/5 to-accent-dark/5 p-6 shadow-soft">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-3xl font-bold tracking-widest text-accent-dark">
                {data.referralCode}
              </p>
            ...
            <div className="flex gap-2">
              ...
              <a
                href={`https://wa.me/?text=${encodeURIComponent(`Hey! I use DCDH Empire for zero-brokerage property search in Jaipur. Use my referral code *${data.referralCode}* when you sign up and we both earn rewards! https://dcdhempire.com`)}`}
                target="_blank"
              ...
              icon={
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M21 12a2.25 2.25 0 00-2.25-2.25H15a3 3 0 110-6h5.25A2.25 2.25 0 0121 6v6zm0 0v6a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 18V6a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 6v6z"
                  />
                </svg>
              }
            />
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs font-medium text-muted">
                  Friend&apos;s Phone Number (WhatsApp)
                </label>
                <input
                  type="tel"
              ...
              <p className="text-xs text-muted">
                We&apos;ll track when they sign up and close a deal. You earn
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, propertyCategory: e.target.value })
                    }
                    className="w-full rounded-xl border border-border bg-surface-white px-4 py-2.5 text-sm text-body outline-none focus:border-accent"
                  >
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, ownerContactNumber: e.target.value })
                  }
                  placeholder="+91 98765 43210"
                  className="w-full rounded-xl border border-border bg-surface-white px-4 py-2.5 text-sm text-body outline-none focus:border-accent"
                />
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setForm({ ...form, propertyNotes: e.target.value })
                  }
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div className="rounded-2xl border border-border bg-surface-white p-12 text-center shadow-soft">
            <svg
              className="mx-auto mb-3 h-10 w-10 text-border"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={1}
            ...
            <p className="mt-1 text-xs text-muted">
              Click &quot;New Referral&quot; to get started!
            </p>
          </div>
        ) : (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[r.status] || "bg-gray-100 text-gray-600"}`}
                    >
                      {r.status.replace("_", " ")}
                    </span>
                  </div>
                    ...
                    <div className="mt-2">
                      ...
                      <p className="text-xs text-muted">
                        ...
                        {r.property_type && (
                          <span>
                            {" "}
                            &middot; {r.property_type.replace("_", " ")}
                          </span>
                        )}
                        {r.owner_contact_number && (
                          <span> &middot; Owner: {r.owner_contact_number}</span>
                        )}
                      </p>
                    </div>
                  )}
